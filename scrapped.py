...
parser.add_argument('-u', '--url', default='Use -h Parameter to show Help.', help="Set target url.") # URL

parser.add_argument('-sT', '--scrap_tag', default=defaultIs, help='Scrap Code.      Methods = P, H1, A.') # SCRAP TAG

parser.add_argument('-m', '--method', default='Use -h Parameter to show Help', help='[P, Methods = FULL_TEXT, ID, CLASS] [H1-H6, Methods = NO METHOD] [A, Methods = HREF, CLASS]') # METHOD

parser.add_argument('-i', '--info', default='Use -h Parameter to show Help', help='Informations = ROBOTS.TXT, DNSLOOKUP') # INFO

parser.add_argument('-sH', '--show_help', default='Use -h Parameter to show Help', help='Show Help of a Parameter ex: -sH P') # SHOW HELP
...
if args.url != 'Use -h Parameter to show Help.':
    ...
    if getTarget.status_code == 404 or getTarget.status_code == 403:
        ...
        if args.info == 'ROBOTS.TXT':
            ...
            print(getDns.text.replace('A :', f'{WColor}[{LGColor}A{WColor}] :').replace('MX :', f'{WColor}[{LYColor}MX{WColor}] :').replace('NS :', f'{WColor}[{LCColor}NS{WColor}] :').replace('TXT :', f'{WColor}[{LBColor}TXT{WColor}] :').replace('SOA :', f'{WColor}[{LRColor}SOA{WColor}] :'))
...
if args.show_help == 'P':
    ...
    print('- Example Usage = -i ROBOTS.TXT')
    print()
    print('Full Usage')
